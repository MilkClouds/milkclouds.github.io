...
$\vec a,\vec b$ 모두 영벡터일 때 $\overrightarrow {OR}=\dfrac{2\overrightarrow{OO'}}{3}$이므로 R은 중심이 $(4,0)$이고, $\vec a,\vec b$ 방향이 서로 반대일 때 반지름이 최소($1 \over 3$)이고 $\vec a,\vec b$ 방향이 서로 같을 때 반지름이 최대($1$)가 되는 원을 그린다.   
